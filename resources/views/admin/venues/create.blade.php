...
<div class="py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            ...
            <p class="text-gray-400">Add a venue that can be used in admin-managed event timings</p>
        </div>
        ...
        <div class="card-luxury rounded-lg p-8 max-w-2xl">
            <form action="{{ route('admin.venues.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex gap-4 pt-6 border-t border-slate-600">
                    ...
                    <a href="{{ route('admin.venues.index') }}" class="px-6 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg font-medium transition">
...
searchInput.addEventListener('input', function (e) {
    ...
    searchTimeout = setTimeout(() => loadCities(query), 300);
});

function loadCities(query) {
    fetch(`{{ route('admin.search-cities') }}?q=${encodeURIComponent(query)}`)
        ...
        .then(data => {
            ...
            if (!data || data.length === 0) {
                ...
                data.forEach(city => {
                    ...
                    div.className = 'px-4 py-3 hover:bg-slate-700 cursor-pointer border-b border-slate-700 text-sm text-white transition-colors';
        ...
        .catch(() => {
            suggestions.innerHTML = '<div class="px-4 py-3 text-red-400 text-sm">Error loading suggestions</div>';
            suggestions.classList.remove('hidden');
        });
}
